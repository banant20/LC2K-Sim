...
int main(int argc, char *argv[]) {
    ...
    // all pipeline registers should be initialized to noop
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        // newState = machine state @ end of cycle
        newState = state;
        newState.cycles++;
        ...
        /* ---------------------- ID stage --------------------- */
        // PC + 1 & instruction bits --> IDEX register
        newState.IDEX.instr = state.IFID.instr;
        newState.IDEX.pcPlus1 = state.IFID.pcPlus1;
        
        if ((field1(state.IDEX.instr) == field1(newState.IDEX.instr) ||
        ...
        /* ---------------------- EX stage --------------------- */
        // PC + 1 & instruction bits --> EXMEM branch
        ...
        if (opcode(state.EXMEM.instr) == BEQ) {
            // dont consider instructions executed so far and reset pc if branch actually taken
            if (state.EXMEM.eq == 1) {
